...
import { SHAPES, getShape, type Shape3D } from "@/lib/shapes";
...
export function ShapesExplorer() {
  ...
  const { scan, stop, activeShapeId, frame, error } = useShapeSonification();
  const [selectedId, setSelectedId] = useState<string>(SHAPES[0].id);
  const buttonRefs = useRef<Record<string, HTMLButtonElement | null>>({});

  const previews = useMemo(
    () => new Map(SHAPES.map((shape) => [shape.id, shape.buildPath(PREVIEW_SAMPLES)])),
    [],
  );
  ...
  // Deep link and voice navigation land here: /shapes?shape=torus
  ...
  useVoiceHandler((command) => {
    ...
    if (command.type === "help") {
      announce(
        "Say select the sphere, play the torus, or scan the cube. Say stop to end the scan, or where am I to hear the current shape.",
      );
      return true;
    }
    return false;
    ...
    <div className="mx-auto max-w-7xl px-6 py-10">
      <header className="max-w-3xl">
        ...
        <p className="mt-3 text-zinc-400">
          Ten solids, each with an automatic spatial scan. A tone travels along the surface while a
          3D panner places it around your head: height becomes pitch, depth becomes distance, and
          every sharp turn is an edge. Headphones strongly recommended.
        </p>
        <p className="mt-3 text-sm text-zinc-500">
          Press a number key from <kbd className="rounded bg-zinc-800 px-1.5 py-0.5 text-xs">1</kbd> to{" "}
          <kbd className="rounded bg-zinc-800 px-1.5 py-0.5 text-xs">0</kbd> to scan a shape, or{" "}
          <kbd className="rounded bg-zinc-800 px-1.5 py-0.5 text-xs">Esc</kbd> to stop.
        </p>
      </header>
        ...
        <aside aria-labelledby="detail-heading" className="lg:sticky lg:top-24 lg:self-start">
          <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 p-6">
            ...
            <div className="mt-6 flex gap-3">
              ...
              >
                {activeShapeId === selected.id ? "Restart scan" : "Play scan"}
              </button>
              <button
                type="button"
                onClick={() => {
